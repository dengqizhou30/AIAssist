...
namespace GameAssistEfficientdet
{
    /// <summary>
    /// opencv及c#封装库，目前还不支持tf2.x模型，只能把Efficientdet模型转换为早期模型，导致输入shape固定不支持网络reshape，导致性能无法提升。
    /// 目前只能放弃这个模型，等opcv及OpenCvSharp发展升级。
    /// </summary>
    ...
    class ScreenDetection
    {
        ...
        **/
        private const string configFile = @"data\efficientdet\d0.pbtxt";
        private const string modelFile = @"data\efficientdet\d0.pb";
        private const string labelFile = @"data\efficientdet\coco.names";
            ...
            if (showPicture && mainForm != null && outPictureBox != null && mainForm.Visible)
            {
                if(frameMat != null)
                {
                    foreach(ObjectPosRect rect in objectPosRects)
                    {
                        ...
                        string msg = (string)this.labelNames.GetValue(rect.classid) + ", " + rect.confidence.ToString("F2") + ", " + toltalMillis;
